...
def Run(ra, dec, width, name=None, out_dir=None, temp_dir=None, replace=False, flux=True, thumbnails=False, montage_path=None):
    ...
    for i in np.random.permutation(range(name_list.shape[0])):
        ...
        for band in bands_dict.keys():
            pool.apply_async(IRIS_Query, args=(name, ra, dec, width, band, bands_dict, temp_dir, montage_path,))
...
def IRIS_Query(name, ra, dec, width, band, bands_dict, temp_dir, montage_path=None):
    ...
    iris_fields = np.arange(1,341).astype(str)
    ...
    if not os.path.exists(mImgtbl_tablepath):
        print('Computing overlap of '+bands_dict[band]['wavelength']+'um IRAS-IRIS plates')
    ...
    print('Inspecting IRAS-IRIS '+bands_dict[band]['wavelength']+'um data for '+name)
