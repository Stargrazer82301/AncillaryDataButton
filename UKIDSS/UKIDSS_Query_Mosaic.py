...
# Define function to wget UKIDSS tiles
def UKIDSS_wget(tile_url, tile_filename, verbose=False):
...
def UKIDSS_Montage(name, ra, dec, pix_width, map_width, band, input_dir, out_dir):
    print 'Montaging '+name+'_UKIDSS_'+band
    ...
    montage_wrapper.wrappers.reproject(os.path.join(input_dir,'Montage_Temp','mosaic.fits'), os.path.join(out_dir,name+'_UKIDSS_'+band+'.fits'), header=os.path.join(input_dir,str(name)+'_HDR'), exact_size=True, cleanup=True, silent_cleanup=True)
    #ChrisFuncs.FitsCutout( os.path.join(input_dir,'Montage_Temp','mosaic.fits'), ra, dec, 0.5*map_width*3600.0, reproj=False, variable=False, outfile=os.path.join(out_dir,name+'_UKIDSS_'+band+'.fits'), parallel=False )
    #os.system('mExec -r '+input_dir+' -f '+os.path.join(input_dir,str(name)+'_HDR')+' -o '+os.path.join(out_dir,name+'_UKIDSS_'+band+'.fits'))
...
if __name__ == "__main__":
    ...
    # Define paths
    in_dir = '/home/sarumandata2/spx7cjc/NESS/Ancillary_Data/UKIDSS/Temporary_Files/'#'/home/herdata/spx7cjc/DustPedia/UKIDSS/Temporary_Files/'
    out_dir = '/home/sarumandata2/spx7cjc/NESS/Ancillary_Data/UKIDSS/Mosaics/'
    ...
    bands = ['Z','Y','J','H','K']
    ext_bands = {'1':'Z','2':'Y'}
    vega_to_AB = [-0.528, -0.634, -0.938, -1.379, -1.900]
    ...
    already_file = '/home/sarumandata2/spx7cjc/NESS/Ancillary_Data/UKIDSS/UKIDSS_Already_Processed_List.dat'
    ...
    # Loop over each source, with handling for problematic cases
    for i in np.random.permutation( np.arange( 0, ness_cat.shape[0] ) ):
        name = name_list[i]
        ra = ra_list[i]
        dec = dec_list[i]
        width = 0.25
        ...
        print 'Querying UKIDSS server using SIA protocol'
        query_success = False
        while not query_success:
            try:
                query_url = 'http://wfaudata.roe.ac.uk/ukidssdr10-siap/?POS='+str(ra)+','+str(dec)+'&SIZE='+str(width)+'&INTERSECT=OVERLAPS'
        ...
        for j in range(0, len(query_table)):
            ...
            # Alter URL so that full UKIDSS MEF file is retrieved, but still keep track of the extension of interest
            tile_extnum = query_table[j]['Reference'].split('http://surveys.roe.ac.uk/wsa/cgi-bin/siap.cgi?extNum=')[1].split('&file=djoser:')[0]
            tile_url = 'http://wsa.roe.ac.uk/cgi-bin/fits_download.cgi?file=djoser:'+query_table[j]['Reference'].split('extNum=')[1].split('djoser:')[1]
            tile_filename = os.path.join( in_dir, name, 'Raw', tile_extnum+'_'+tile_url.split('/')[-1:][0] )

            # Commence parallel download
            #UKIDSS_wget( tile_url, os.path.join(in_dir,name,'Raw',tile_filename) )
            dl_pool.apply_async( UKIDSS_wget, args=( tile_url, os.path.join(in_dir,name,'Raw',tile_filename), ) )
        dl_pool.close()
        dl_pool.join()
        ...
        for j in range(0, len(query_table)):
            for raw_file in os.listdir( os.path.join( in_dir, name, 'Raw') ):
                if '_'.join(raw_file.split('_')[1:])==query_table[j]['Reference'].split('/')[-1:][0]:
        ...
        for b in range(0, len(bands)):
            ...
            for raw_fits in os.listdir( os.path.join( in_dir, name, band) ):
                ...
                # Load in appropriate file extensions, and record pixel size
                raw_fitsdata = astropy.io.fits.open( os.path.join( in_dir, name, band, raw_fits ) )
                meta_header = raw_fitsdata[0].header
                raw_image = raw_fitsdata[raw_exten].data
                raw_header = raw_fitsdata[raw_exten].header
                raw_wcs = astropy.wcs.WCS(raw_header)
                raw_pix_width = np.sqrt( raw_wcs.wcs.cd[0,0]**2.0 + raw_wcs.wcs.cd[0,1]**2.0 )
                if raw_pix_width<pix_min[b]:
                    pix_min[b] = raw_pix_width

                # Remove outer 3 arcsec of bad pixels from edge of map
                pix_border_width = np.int( np.round( 6.0 / ( 3600.0 * pix_min[b] ) ) )
                raw_image[:pix_border_width,:] = np.NaN
                raw_image[-pix_border_width:,:] = np.NaN
                raw_image[:,:pix_border_width] = np.NaN
                raw_image[:,-pix_border_width:] = np.NaN

                # Use header information and Vega-to-AB conversions to work out true zero-point magnitude, as per prescription here: http://www.ast.cam.ac.uk/ioa/research/vdfs/docs/catalogues.pdf
                meta_exptime = meta_header['EXP_TIME']
                meta_airmass = 0.5 * ( meta_header['AMSTART'] + meta_header['AMEND'] )
                meta_extinct = raw_header['EXTINCT']
                meta_mag_zpt = raw_header['MAGZPT']
                mag_zpt = meta_mag_zpt - (2.5*np.log10(1/meta_exptime)) - (meta_extinct*(meta_airmass-1.0)) - vega_to_AB[b]

                # Set all pixels to have units of AB mag, using the calculated zero-point magnitude
                out_image = mag_zpt - ( 2.5 * np.log10(raw_image) )

                # Convert pixel units to Jy
                out_image = ChrisFuncs.ABMagsToJy(out_image)

                # Convert pixel units to Jy/sqdeg (switching to surface brightness here becuase different UKIDSS surveys use different pixel sizes)
        ...
        while not complete:
            ...
            try:
                ...
                for b in range(0, len(bands)):
                    if bands_data[b]==False:
                        continue
                    band = bands[b]
                    pool.apply_async( UKIDSS_Montage, args=(name, ra, dec, out_pix_width, width, band, os.path.join( in_dir, name, band ), out_dir,) )
                    #UKIDSS_Montage(name, ra, dec, 0.4, width, band, os.path.join( in_dir, name, band ), out_dir)#3600.0*pix_min[b]
                pool.close()
                pool.join()
                complete = True
            ...
            # Handle failures
                ...
                time.sleep(5)
        if complete==False:
            print '##### ABORTING TARGET '+name+' AFTER 10 FAILED MOSAICING ATTEMPTS #####'
            shutil.rmtree( os.path.join( in_dir, name ) )
            time_list.append(time.time())
        ...
        for band in bands:
            if os.path.exists( os.path.join(out_dir,name+'_UKIDSS_'+band+'.fits') ):
                uncorr_image, uncorr_header = astropy.io.fits.getdata( os.path.join(out_dir,name+'_UKIDSS_'+band+'.fits'), header=True )
                ...
                astropy.io.fits.writeto( os.path.join(out_dir,name+'_UKIDSS_'+band+'.fits'), corr_image, header=uncorr_header, clobber=True )
