...
for i in range(0, name_list.shape[0]):
    ...
    # Check if VISTA data present
    nomap = 0
    for band in bands:
        if not os.path.exists(in_dir+str(name)+'_VISTA_'+str(band)+'.fits'):
            nomap += 1
    if nomap==5:
        continue
    ...
    for b in range(0, len(bands)):
        ...
        if os.path.exists( os.path.join(out_dir, name+'_VISTA_'+band+'.fits' ) ):
            print 'Map for '+name+'_VISTA_'+band+'.fits already processed'
            continue

        # Check if input data exists exists; if not, skip
        if not os.path.exists( os.path.join( in_dir, name+'_VISTA_'+band+'.fits' ) ):
            print 'No data for '+name+'_VISTA_'+band+'.fits'
            continue

        # Read in map, skipping if file is corrupt
        try:
            in_image, in_header = astropy.io.fits.getdata( os.path.join( in_dir, name+'_VISTA_'+band+'.fits' ), header=True )
        except:
            print 'File '+name+'_VISTA_'+band+'.fits appears to be corrupted'
            continue
        ...
        cutout_header.set('TELESCOP', 'VISTA', 'Telescope that made this observation')
        cutout_header.set('INSTMNT', 'VIRCAM', 'Instrument used for this observation')
        ...
        image_cutout_hdulist.writeto( os.path.join(out_dir, name+'_VISTA_'+band+'.fits' ), clobber=True)

        # Make thumbnail image of cutout
        if thumbnails:
            thumb_out = aplpy.FITSFigure(out_dir+name+'_VISTA_'+band+'.fits')
            ...
            thumb_out.save( os.path.join(out_dir, 'Thumbnails', name+'_VISTA_'+band+'.png') )
            thumb_out.close()
