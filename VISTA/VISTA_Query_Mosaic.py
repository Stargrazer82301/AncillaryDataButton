...
if __name__ == "__main__":
    ...
    # Define paths
    in_dir = '/home/sarumandata2/spx7cjc/NESS/Test_Sample/VISTA/Temporary_Files/'
    out_dir = '/home/sarumandata2/spx7cjc/NESS/Test_Sample/VISTA/Mosaics/'
    ...
    already_file = '/home/sarumandata2/spx7cjc/NESS/Test_Sample/VISTA/VISTA_Already_Processed_List.dat'
    ...
    for i in range(0, NESS_cat.shape[0]):#np.where(name_list=='NESS27')[0]:
...
"""
# Remove outer 1 arcmiute of shallow data from edge of map
pix_border_width = np.int( np.round( 60.0 / ( 3600.0 * pix_min[b] ) ) )
...
"""
# Create cutouts of maps that cover only region of interest, to speed up processing, cut save the original header
raw_fitsdata = astropy.io.fits.open( os.path.join( in_dir, name, band, raw_fits ) )
raw_image = raw_fitsdata[raw_exten].data
raw_header = raw_fitsdata[raw_exten].header
raw_wcs = astropy.wcs.WCS(raw_header)
raw_fitsdata.close()
cutout_obj = astropy.nddata.utils.Cutout2D( raw_image, astropy.coordinates.SkyCoord(ra, dec, unit='deg'), 1.414*width*astropy.units.degree, wcs=raw_wcs, mode='trim', copy=True )
cutout_image = cutout_obj.data
cutout_wcs = cutout_obj.wcs
cutout_header = cutout_wcs.to_header()
astropy.io.fits.writeto( os.path.join( in_dir, name, band, raw_fits ), cutout_image, header=cutout_header, clobber=True )
"""
